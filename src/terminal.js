 ...
 * InteractiveTerminal
 * 
 * Manages an xterm.js terminal that provides a terminal-like experience
 * for batch-mode WebAssembly execution (BrowserCC + WASI).
 *
 * Architecture:
 *   Phase 1 (Input Collection): After "Run" is clicked, the terminal enters
 *   interactive mode. It shows a prompt and waits for the user to type each
 *   stdin line, pressing Enter to submit.
 *
 *   Phase 2 (Execution): Once the user signals end-of-input with Ctrl+D on
 *   an empty line (or the program explicitly uses a finite number of scanf),
 *   the collected stdin is passed to the WASI batch execution. The terminal
 *   then renders the final output, interleaving echoed input at the correct
 *   positions using the EchoingStdin mechanism.
 *
...
export class InteractiveTerminal {
   ...
   * the terminal as typed (normal terminal behaviour).
   *
   * End-of-input is signalled by pressing Ctrl+D on an EMPTY line, which
   * simulates a Unix EOF on stdin.
   *
   * @returns {Promise<string>} Resolves with all collected lines joined by '\n'.
   */
  collectStdin() {
